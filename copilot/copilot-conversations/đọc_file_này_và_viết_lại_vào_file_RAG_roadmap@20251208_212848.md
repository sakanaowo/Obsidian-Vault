...
topic: "RAG Roadmap File Rewrite"
...
**user**: đọc file này và viết lại vào file [[RAG roadmap]] dựa theo system instruction 
[Context: Notes: RAG-roadmap/RAG roadmap.md, assets/documents/RAG roadmap.pdf]
[Timestamp: 2025/12/08 21:28:48]

**ai**: [Bản gốc]

(Toàn bộ nội dung note [[RAG roadmap]] trong context.)

---

[Phân tích lỗi/điểm yếu]

- Văn phong hiện tại:
  - Mang tính hướng dẫn thực hành, gần gũi, có nhiều câu ví dụ đời thường, ví von (“build a skyscraper on a swamp”, “Garbage In, Garbage Out”).
  - Nhiều cụm từ khẩu ngữ, marketing (“The Industry Standard”, “It works exceptionally well”, “The Big Players”).
- Về mặt học thuật:
  - Chưa nhất quán về cấu trúc trình bày: có đoạn dùng gạch đầu dòng chi tiết, có đoạn mô tả dài, có chỗ giải thích khái niệm, chỗ khác chỉ liệt kê mà không định nghĩa.
  - Một số mục chưa nêu rõ mục tiêu hay định nghĩa trước khi liệt kê ví dụ (ví dụ: Agentic RAG, Corrective RAG).
  - Chưa tách bạch rõ: định nghĩa – vai trò – ví dụ – ứng dụng.
- Về cấu trúc:
  - Cấu trúc pha (Phase 1–5) tốt, nhưng mỗi pha có độ chi tiết không đồng đều.
  - Một số mục có thể chuẩn hóa thêm: định nghĩa ngắn gọn, sau đó bullet rõ ràng.
- Mức độ “Plain Language”:
  - Nhiều thuật ngữ đã được giải thích tốt, nhưng vẫn tồn tại cách diễn đạt mang tính khẩu hiệu/ẩn dụ, chưa hoàn toàn trung tính.

---

[Bản viết lại tối ưu]  
(Đây là bản rewrite hoàn chỉnh để thay thế nội dung trong [[RAG roadmap]], giữ nguyên cấu trúc pha, nhưng chuyển sang giọng văn học thuật, khách quan và mạch lạc.)
...
[Timestamp: 2025/12/08 21:30:42]

**user**: đọc các file trong toàn bộ vault này và đề xuất chuẩn hóa frontmatter yaml cho system instruction 
[Timestamp: 2025/12/08 21:35:04]

**ai**: Để chuẩn hóa frontmatter YAML cho các note system instruction (ví dụ như [[Academic Rewrite]] hoặc các note “hướng dẫn dùng Copilot”), có thể dùng một schema thống nhất như sau:

## 1. Mẫu frontmatter YAML đề xuất

```yaml
---
type: system-instruction        # Phân biệt với note học thuật, dự án, log,...
role: knowledge-editor          # Hoặc: knowledge-architect, rag-architect,...
purpose: "Chuẩn hóa văn phong học thuật và cấu trúc note"  # Mục tiêu ngắn gọn

scope:
  - rewriting                   # Các loại tác vụ chính
  - summarization
  - structuring
  - metadata-design

status: active                  # active | draft | deprecated
version: 1.0.0                  # Để quản lý thay đổi
created: 2025-12-08             # yyyy-mm-dd
updated: 2025-12-08

language: vi                    # vi | en | mixed
audience:
  - self-notes                  # self-notes | papers | blog | documentation
  - academic

style:
  tone: academic                # academic | plain | mixed
  clarity: plain-language       # nhấn mạnh diễn giải dễ hiểu
  use_headings: true
  use_bullets: true
  highlight_keywords: true      # dùng **bold** cho khái niệm chính
...
copilot:
  enabled: true
  command-context-menu-enabled: true
  command-slash-enabled: true
  default-model: "gpt-4.x"
  last-used: 0
---
```

## 2. Nguyên tắc áp dụng trong vault hiện tại

Dựa trên các note bạn đang có:

- Với **note hướng dẫn / prompt hệ thống**  
  - Ví dụ: [[Academic Rewrite]] → `type: system-instruction`, `role: knowledge-editor`, `purpose: "Viết lại văn bản theo văn phong học thuật"`.
- Với **note lộ trình / MOC kiến thức** (ví dụ [[RAG roadmap]]):
  ```yaml
  ---
  type: roadmap
  domain: rag
  ...
  ---
  ```
- Với **note bài báo / chương luận văn** (nhóm DragonFruit Paper: [[Introduction]], [[Propose method]], [[Model Description]], [[Experimental result and analysis]], [[Conclusion]], [[Chapter summary]]):
  ```yaml
  ---
  type: paper-section
  project: dragonfruit-scl
  section: introduction          # hoặc: method | model-description | experiment | conclusion
  status: finalized              # draft | in-review | finalized
  language: vi
  tags:
...
## 3. Gợi ý tổ chức note system instruction trong vault

- Thư mục:
  - `copilot/copilot-custom-prompts/`  
    - Chứa toàn bộ note `type: system-instruction`.
- Tag:
  - Dùng chung `system`, `instruction`, và thêm `obsidian`, `copilot` để dễ filter.
- Liên kết:
  - Mỗi system instruction nên có trường `crosslink_to` trỏ tới:
    - Các template quan trọng (ví dụ template bài báo, template roadmap).
    - Các note nền tảng như [[RAG roadmap]], [[Attention is all you need]], [[Batch Normalization]], [[Phương sai]].

Nếu bạn muốn, tôi có thể viết trực tiếp frontmatter YAML hoàn chỉnh cho từng note cụ thể: [[RAG roadmap]], [[Academic Rewrite]], và một note trong bộ DragonFruit (ví dụ [[Introduction]]) để bạn copy-paste.
[Timestamp: 2025/12/08 21:35:56]

**user**: vậy viết lại một instruction cho system với bổ sung frontmatter chuẩn hóa 
[Timestamp: 2025/12/08 21:38:12]
